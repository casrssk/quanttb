...
'''
    ...
    def __init__(self, fileName, hardfilter=True, include=None, data=None):
        ...
        if data:
            ...
            logging.error(
                'In order to make snpsets, must input either a vcf file or a .snps file')
            sys.exit()
        self.masked = False
        self.findres()
...
#                     nucleo = "N"
                        ...
                        if sum(BC) != 0:
                            ...
                            if any([i / float(sum(BC)) > thresh for i in BC]):
                                if base[4] == ".":
                                    ...
                                    nucleo = base[4] if hard else {
                                        base[4]: 1}  # switching to frequency
                                    cov = {base[4]: max(BC)}
                            elif hard:
                                nucleo = "N"
                            else:
                                cov = {nuc: count for nuc, count in zip(
                                    ["A", "C", "G", "T"], BC) if count != 0}
                                # convert to frequency
                                nucleo = {
                                    nuc: (cov[nuc] / float(sum(cov.values()))) for nuc in cov.iterkeys()}
                        else:
                            nucleo = "N"
                    else:
                        nucleo = "N"
        ...
        # saving where the badlocations are
        self.badpos = [pos for pos, snp in zip(positions, nucs) if snp == "N"]
        self.snptable = {pos: snp for pos, snp in zip(
            positions, nucs) if snp != "N"}
        if not hard:
...
#         self.badpos = [x for x in snptable if in_range(x)]
    ...
    def getsnpinfo(self):
        ...
        # self.ambpos = [y for y in self.snptable if
        # isinstance(self.snptable[y], dict)]
        if self.hardfilter == True:
            ...
            for snp in self.snptable:
                ...
                if all([i / float(sum(BC)) < 0.95 for i in BC]):
                    amb.append(snp)
            self.ambpos = amb
            self.amcov = sum([sum(x.values()) for x in self.snptable.values(
            )]) / self.snpcounts if self.snpcounts != 0 else 0
    ...
    def removesnps(self, snpdict, covremove=None, refremove=None):
        if covremove is None:
            covremove = 0.5
        if self.hardfilter:
            self.snptable = {snp: nuc for snp, nuc in self.snptable.items(
            ) if snp not in snpdict or nuc != snpdict[snp]}
        else:
            if refremove:
                self.refdict = {snp: nuc for snp,
                                nuc in self.refdict.items() if snp not in snpdict}
            for pos in self.snptable.keys():
                if pos in snpdict:
                    nuc = snpdict[pos]
                    if nuc in self.snptable[pos]:
                        if covremove is not None:
                            self.snptable[pos][nuc] = max(
                                self.snptable[pos][nuc] - covremove, 0)
                            # removing partial snps this is wrong needs to be
                            # fixed
                            if self.snptable[pos][nuc] * sum(self.covtable[pos].values()) < self.covtable[pos][nuc]:
                                # if round(self.snptable[pos][nuc] *
                                # self.covtable[pos][nuc]) == 0:
                    ...
                    if pos not in self.refdict:
                        continue
                    if pos in refremove:
                        # removing bad positions from snpsample based on cov
                        self.snptable[pos] = {
                            nuc: max(val - covremove, 0) for nuc, val in self.snptable[pos].items()}
                        self.snptable[pos] = {
                            nuc: val for nuc, val in self.snptable[pos].items() if val > 0.05}
                        self.snptable[pos] = {nuc: val for nuc, val in self.snptable[pos].items(
                        ) if val * sum(self.covtable[pos].values()) < self.covtable[pos][nuc]}
                    else:
                        rfnuc = self.refdict[pos]
                        if rfnuc in self.snptable[pos]:
                            self.snptable[pos][rfnuc] = max(
                                self.snptable[pos][rfnuc] - covremove, 0)
                            # removing partial snps
                            if self.snptable[pos][rfnuc] * sum(self.covtable[pos].values()) < self.covtable[pos][rfnuc]:
                                # if round(self.snptable[pos][rfnuc] *
                                # self.covtable[pos][rfnuc]) == 0:
        ...
        for pos in resloc:
            ...
            if len(res.intersection(samploc)) == len(samploc):
                ...
                if items[top] > 0.9:
                    ...
                    suspcnt = 1 - respcnt
                    resistances[pos] = "hetres, res: " + \
                        str(round(respcnt, 2)) + '% sus: ' + \
                        str(round(suspcnt, 2)) + "%"
            else:
...
def iteration(db, snpsampleset, iterationmax=8, withref=True,  maxthresh=0.15):
    ...
    If withref is true, it will ensure the t reference is included in the db'''
    ...
    logging.info("Comparing " + snpsampleset.sample +
                 " against " + str(len(db.sampnames)) + " in database")
    ...
    # matrix subsetted by only the snps that are knwon in the sample
    thisit_mat = thedb.matrix[:, knownlocs]
    thisit_loc = thedb.matloc[knownlocs]

    covlist = np.array([sum(thesample.covtable[x].values())
                        for x in thisit_loc])

    # removing genomes that don't have at least 15 snps in common with sample
    if 'H37rv' in thedb.matrows:
        h37 = thisit_loc[
            (thisit_mat[np.where(db.matrows == 'H37rv')[0], ] == True)[0]]
    ...
    # initializing the coverage threshold for a snp to be detected in sample
    # to low value
    ...
    # calculating current coverage of database in sample excluding purely
    # reference locations
    ...
    # iterationmax has not been reached, and the number of passing snps in
    ...
    while(len(np.unique(thisit_loc)) > 15 and maxvalue > maxthresh and iterations < iterationmax and len(passing) > 0):
        ...
        # frequency of SNPs for every strain in database
        freqmat = thisit_mat * sampfreqs_known
        samepos = freqmat.sum(axis=1)
        # Number of SNPs present at any frequency for every strain in database
        fullpos = np.count_nonzero(freqmat, axis=1)
        sampscore = samepos / len(set(thisit_loc))
        sampscore[np.where(fullpos < 16)] = 0

        logging.debug('sample average cov: ' + str(scov) +
                      ' covthresh ' + str(covthresh))

        # for the calculation of percent overlap, only considering SNPs that
        # pass vcoverage threshold
        allowed = (np.where((sampfreqs_known > 0.05)
                            & (sampcovs_known > covthresh))[0])
        top = np.count_nonzero(freqmat[:, allowed], axis=1)
        # Removing incidence of SNPs present at less than 16
        top[np.where(fullpos < 16)] = 0
        top = top.astype(float)
        pcntc = np.divide(top, snpcounts, out=np.zeros_like(
            top), where=snpcounts != 0)

        # calculating the average frequency and coverage per strain of SNPs in
        # sample
        totfreqs = freqmat.sum(axis=1)
        amcov = np.divide(totfreqs, snpcounts,
                          out=np.zeros_like(top), where=snpcounts != 0)
        covmat = thisit_mat * sampcovs_known
        totcovs = covmat.sum(axis=1)
        amdepth = np.divide(totcovs, snpcounts,
                            out=np.zeros_like(top), where=snpcounts != 0)
        ...
        topfive, topfive_totscore, topfive_sampscore, topfive_pcntc = [
            x[ind] for x in [thisit_refs, totscore, sampscore, pcntc]]
        ...
        # if it doesnt, check whether anyhing else in the top 5 does. This
        # removes spuriously identified strains
        changelist = OrderedDict()
        for poss in ind:
            query = set(map(tuple, thisit_cols[~thisit_mat[
                np.where(thisit_refs == thisit_refs[poss])[0]][0]]))
            if thisit_refs[poss] == 'H37rv':
                query = set([x for x in query if x[0]
                             not in thedb.sample('H37rv').snptable.keys()])
        ...
        if change < .015 and scov > 10:
            for poss in changelist:
                ...
                if change > .015:
                    logging.debug(identified + ' has been changed to ' + thisit_refs[
                        poss] + ' with change of ' + str(change) + ' and ncovin of ' + str(changelist[poss]))
                    # print identified + ' has been changed to ' +
                    # thisit_refs[poss] + ' with change of ' +   str(change) +
                    # ' and ncovin of ' +  str(changelist[poss])
                    ind[0] = poss
        ...
        # if the maxvalue of this iteration passes threshold, remove the SNPs
        # from database and sample.
        if maxvalue > maxthresh:
            identified = thedb.sample(identified)
            identified.totscore, identified.samepos, identified.fullpos, identified.samplescore, identified.percentcommon, identified.amcov, identified.amdepth, identified.snpcounts = [
                x[ind[0]] for x in [totscore, samepos, fullpos, sampscore, pcntc, amcov, amdepth, snpcounts]]

            # recalculating covthresh to make sure next identified strain has
            # coverage at least 5% of this one
            if scov < 25:
            ...
            sampfreqs_known[thisit_idsnps] = np.maximum(
                sampfreqs_known[thisit_idsnps] - amcov[ind[0]], 0)

            thisit_remsnps = thisit_idsnps[(sampfreqs_known[
                thisit_idsnps] * covlist[thisit_idsnps] < sampcovs_known[thisit_idsnps])]
            thisit_remsnps = np.append(thisit_remsnps, thisit_idsnps[
                (sampfreqs_known[thisit_idsnps] < .05)])
            thisit_remsnps = np.unique(thisit_remsnps)

            logging.debug('removed snps from sample :' + str(len(thisit_remsnps)
                                                             ) + ' snps in id strain: ' + str(len(thisit_idsnps)))
            ...
            # matrix subsetted by only the snps that are knwon in the sample
            thisit_mat = thisit_mat[:, knownlocs]
            thisit_loc = thisit_loc[knownlocs]
            covlist = covlist[knownlocs]

            if 'H37rv' in thisit_refs:
                h37 = thisit_loc[
                    (thisit_mat[np.where(thisit_refs == 'H37rv')[0], ] == True)[0]]
                noth37 = np.where(~np.isin(thisit_loc, h37))[0]
                remaining = np.count_nonzero(thisit_mat[:, noth37], axis=1)
                passing = np.where((remaining >= 15) +
                                   (thisit_refs == 'H37rv'))[0]
            else:
                remaining = np.count_nonzero(thisit_mat, axis=1)
            ...
            logging.debug(identified.sample + ' totscore: ' + str(topfive_totscore[0]) + 'amcov: ' + str(identified.amcov) + ' snpcounts: ' + str(
                identified.snpcounts) + ' samepos: ' + str(identified.samepos) + ' fullpos: ' + str(identified.fullpos))
            strains.append(identified)

            covin = ncovin
            iterationres[str(iterations)] = [thisit_cols,
                                             thisit_refs, thisit_mat, sampfreqs_known, covlist]
            if covin == 0:
                maxvalue = 0

        iterationres[str(iterations)] = [thisit_cols, thisit_refs,
                                         thisit_mat, sampfreqs_known, covlist]
    ...
    row1 = ["refname", "totscore",  "samplescore", "commonpcnt",
            "finalrel",  "finalcov", "amcov", "amdepth"]
...
def remsubsets(idstrains, samp):
    ...
    for pair in combinations(idstrains, 2):
        ...
        compos = [len([(x, y) for x, y in snps if x in samp.covtable])
                  for snps in un]
        if min(compos) < 16:
            sub = compos.index(min(compos))
            rem += [pair[sub].sample]
            logging.debug('Identified strains: ' +
                          pair[sub].sample + ' is subset of another. Removing')
    rem = list(set(rem))
...
def getsampcov(snps, samp, tot=True):
    ...
    cov = sum([samp.covtable[x][y] for x, y in compos if y in samp.covtable[
        x]]) / float(len(snps)) if len(snps) > 0 else 0
    if not tot:
        den = len([samp.covtable[x][y]
                   for x, y in compos if y in samp.covtable[x]])
        cov = sum([samp.covtable[x][y] for x, y in compos if y in samp.covtable[
            x]]) / float(den) if den != 0 else 0
...
def getpcntcov(snps, samp):
    ...
    pc = len([x for x in pc if x >= 2]) / \
        float(len(snps)) if len(snps) > 0 else 0
    return (pc)
...
def decide(db, otherans, sample, pov=None):
    ...
    close = [len(comsnpget(x) & comsnpget(pov)) / len(comsnpget(pov))
             for x in otherans]
    closerev = [len(comsnpget(x) & comsnpget(pov)) / len(comsnpget(x))
                if len(comsnpget(x)) > 0 else 0 for x in otherans]
...
class Snpdb(object):
    ...
    def getsets(self):
        if isinstance(self.input, list):
            if isinstance(self.input[0], str):
                ...
                raise Exception(
                    "Please provide a list of vcf filenames or pkl filenames or SnpSets")
            self.samplesets = thesets
            self.sampnames = [sett.sample for sett in self.samplesets]
            logging.info('Removing nearby SNPs in each genome of distance ' +
                         str(self.reducedist) + ' to each other')
            self.reducesnpsets(snpdist=self.reducedist)
            if self.filterdist is not None:
                self.counts = None
                self.makematrix()
                logging.info('Filtering genomes in the database on distance ' +
                             str(self.filterdist) + ' to each other')
    ...
    def uniquesnps(self):
        uniques = []
        snps = set().union(*[sett.snptable.items()
                             for sett in self.samplesets])
            ...
            if pos in set2.snptable:
                if nuc == set2.snptable[pos]:
                    ...
                    if nuc in set2.snptable[pos].keys():
                        over.append(pos)
        # to account for situations where ambigous bases match to two different
        # samples
    ...
    def snpfreq(self):
        # unsnps = dict(self.uniquesnps())
        counts = Counter(sum([sett.snptable.keys()
                              for sett in self.samplesets], []))
    ...
    def filtersnpdb(self, thresh=150):
        ...
        nmat = self.matrix.A.astype(float)

        for x in self.matrows:
            nmat[self.matrows == x,  np.isin(
                self.matloc, self.sample(x).badpos)] = np.nan

        # calculating pairwise distance, time consuming
        dm = pdist(nmat, lambda u, v: (np.nansum((u - v)**2)))

        dm = squareform(dm)
        np.fill_diagonal(dm, np.inf)
        k = np.argwhere(dm < thresh)
        close = set()
        for x, y in k:
            close.add(tuple(sorted([self.matrows[x], self.matrows[y]])))

        toremove = []
        close = list(close)
        while len(close) > 0:
            x, y = close[0]
            rem, keep = sorted([x, y], key=lambda l: self.sample(l).snpcounts)
            close = [x for x in close if rem not in x]
            toremove += [rem]

        if len(toremove) > 0:
            logging.info("Removing " + str(len(toremove)) + " references " +
                         ",".join(toremove) + " from db")
            for ref in toremove:
                self.removeref(ref)
        else:
            logging.info("No references need to be removed")
    ...
    def possnpcounts(self, snp=True):
        if not hasattr(self, 'counts'):
            if snp:
                self.counts = Counter(
                    sum([sett.snptable.items() for sett in self.samplesets], []))
            else:
                self.counts = Counter(
                    sum([sett.snptable.keys() for sett in self.samplesets], []))
        return(self.counts)
    ...
    def weighedsnppersample(self):
        counts = self.possnpcounts()
        for sample in self.samplesets:
            sample.weighedsnp = [countallele[snp]
                                 for snp in sample.snptable.items()]

    # removes positions per sample that are within x bases of each other
    def reducesnpsets(self, snpdist=25):
        track = 0
        for ref in self.samplesets:
            ...
            closeby = np.where(
                np.diff(sorted(ref.snptable.keys())) < snpdist)[0]
            closeby = np.append(closeby + 1, closeby)
            toremove = [y for x, y in enumerate(
                sorted(ref.snptable.keys())) if x in closeby]
            ref.badpos += toremove
...
def getvcf(fastas, tempdir="temp", keepint=False):
    ...
    if not os.path.isfile(vcfloc):
        pilon = "java -jar " + \
            resource_filename(Requirement.parse('quanttb'),
                              'quanttb/data/pilon-1.22.jar')

        getvcf = pilon + " --genome " + ref + " --bam " + bamloc + " --output " + \
            name + " --outdir " + tempdir + " --vcf" + " --threads 4 --fix none"
        if not os.path.isfile(bamloc):
            aln = "bwa mem -t 4 " + ref + " " + fastq + \
                " | samtools sort - -T sam_tempsort" + name + " > " + bamloc
    ...
    for command in (aln,  index, getvcf):
        if command != '':
            subprocess32.call(command, shell=True, stderr=vcflog,
                              timeout=10000, stdout=vcflog)
